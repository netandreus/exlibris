<?php
class ZendExtra_OpenId_Provider_Vkontakte extends ZendExtra_OpenId_Provider_Abstract {
    ...
    public function generateUsername($preferredUsername = NULL)
    {
        ...
        if(strpos($identity, 'http://vkontakte.ru/id') !== false) {
            $id = str_replace('http://vkontakte.ru/id', '', $identity);
            $username = $this->_usernamePrefix.self::DELMITTER.$id;
        } else {
            throw new Exception('Wrong Vkontakte identity', 500);
        }
        return $username;
    }
    ...
    public function generateUserData() {
        ...
        $data = array(
            ...
            'name'            => (key_exists('name', $userData['profile']))? $userData['profile']['name']['last_name'].' '.$userData['profile']['name']['first_name'] : $this->generateUsername(),
