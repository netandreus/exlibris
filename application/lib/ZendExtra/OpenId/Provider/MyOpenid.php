<?php
class ZendExtra_OpenId_Provider_MyOpenid extends ZendExtra_OpenId_Provider_Abstract {
    ...
    public function generateUsername($preferredUsername = NULL)
    {
        ...
        if(strpos($identity, 'myopenid.com/') !== false) {
            $id = str_replace('.myopenid.com/', '', $identity);
            $id = str_replace('http://', '', $id);
            $id = str_replace('https://', '', $id);
            $username = $this->_usernamePrefix.self::DELMITTER.$id;
        } else {
            throw new Exception('Wrong MyOpenid identity', 500);
        }
        return $username;
    }
